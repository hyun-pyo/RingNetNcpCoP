...
# vpc resource
resource "ncloud_vpc" "ncp_vpc_002" {
  name            = var.vpc_name
...
resource "ncloud_subnet" "cluster_subnet_01_private" {
  ...
  vpc_no         = ncloud_vpc.ncp_vpc_002.vpc_no # provider.resoruce_subject.variable
  subnet         = cidrsubnet(ncloud_vpc.ncp_vpc_002.ipv4_cidr_block, 8, 1) # 10.0.1.0/24
...
resource "ncloud_subnet" "cluster_subnet_01_private_lb" {
  ...
  vpc_no         =  ncloud_vpc.ncp_vpc_002.vpc_no 
  subnet         = cidrsubnet(ncloud_vpc.ncp_vpc_002.ipv4_cidr_block, 8, 2) # 10.0.2.0/24
  zone           = var.ncp_zone
  network_acl_no = ncloud_network_acl.nacl.id
  subnet_type    = "PRIVATE"
  usage_type     = "LOADB"
...
resource "ncloud_nat_gateway" "nat_gateway" {
  vpc_no      =  ncloud_vpc.ncp_vpc_002.id
  zone        = var.ncp_zone
